...
@Service
//toutes les methodes sont transactional (commit ou rollback)
@Transactional
public class CinemaInitServiceImpl implements ICinemaInitService{
	...
	public void initFilms() {
		double[] durees=new double[] {1,1.5,2,2.5,3}; //tableau de la duree du film
		List<Categorie> categories= categorieRepository.findAll();//la liste des categories 
		Stream.of("faites le plein de bonheur","menteur","mustang","roxane","un mariage de reve").forEach(titreFilm ->{
				Film film = new Film();
	...
	public void initProjections() {
		double[] prices= new double[] {30,50,60,70,90,100};//prix des projections des films
		List<Film> films =filmRepository.findAll();
		villeRepository.findAll().forEach(ville ->{
			ville.getCinemas().forEach(cinema ->{
				cinema.getSalles().forEach(salle ->{
					//dans chaque salle en prend un film aleatoirement
					...
					Film film = films.get(index);
							...
							projection.setPrix(prices[new Random().nextInt(prices.length)]);//prices aleatoire
	...
	public void initTickets() {
		projectionRepository.findAll().forEach(projection ->{
			//nombre de ticket selon nombre de place
			projection.getSalle().getPalces().forEach(place ->{
